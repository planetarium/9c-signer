...
class Signer:
    ...
    @staticmethod
    def __public_key_int_to_eth_address(pubkey: int) -> str:
        """
    ...
    @staticmethod
    def __get_sig_r_s(signature: bytes) -> Tuple[int, int]:
        """
    ...
    @staticmethod
    def __get_sig_v(msg_hash: bytes, r: int, s: int, expected_address: str) -> int:
        """
    ...
    def sign_tx(self, unsigned_tx: bytes) -> bytes:
        ...
        seq = SequenceOf(componentType=Integer())  # type: ignore
...
def derive_address(
    ...
    :param address: Original address to make derived address :param key: Derive Key :param get_byte: If set this
    values to `True`, the return value is converted to checksum encoded string. default: False :return: Derived
    address. (Either bytes or str followed by `get_byte` flag)
    :param get_byte:
    :param key:
    """
    ...
    derived = hmac.new(key_bytes, address_bytes, sha1).digest()
